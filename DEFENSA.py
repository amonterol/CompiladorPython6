...
# ERROR import mal colocado			
def verificar_ganador(tablero, jugador):
    """ Función para verificar si hay un ganador """
    # Verificar filas, columnas y diagonales
    for i in range(3):
        if (all(tablero[i][j] = = jugador for j in range(3)) or       # Filas  ERROR  prueba de operador de comparacion separa los ==
            all(tablero[j][i] == jugador for j in range(3)) or       # Columnas
            all(tablero[i][i] == jugador for i in range(3)) or       # Diagonal principal
            all(tablero[i][2 - i] == jugador for i in range(3))):   # Diagonal secundaria --8
            return True
    return False
...
def juego_gato():
	...
	#  ERROR  prueba de input
	nombre = inp ("Ingrese su nombre: ")
	...
	*-num = 10
    ...
    jugador_cpu = "CPU"
    jugador_actual = "X"  # Empieza el usuario--14
    ...
    while True:
        ...
        if jugador_actual == "X":  # Turno del usuario
            ...
            if tablero[fila][columna] ! = " ": #  ERROR  separa el != por lo tanto no cuenta como comparativos
